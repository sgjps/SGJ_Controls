...
begin
  {$IFDEF MSWINDOWS}
  // Jeśli nie ma sublinii, nie ustawiamy kolumn
  if TileOptions.SubLineCount <= 0 then
  begin
    // Nadal musimy ustawić LVM_SETTILEINFO, ale bez kolumn
    FillChar(TileInfo, SizeOf(TileInfo), 0);
    TileInfo.cbSize := SizeOf(TileInfo);
    TileInfo.cColumns := 0;
    TileInfo.puColumns := nil;

    if AItem >= 0 then
    begin
      TileInfo.iItem := AItem;
  ...
  // Ustawiamy informacje o kolumnach dla każdego elementu
  ...
  begin
    ...
    // Kolumny zaczynają się od indeksu 1 (0 = caption)
...
begin
  inherited Loaded;
  {$IFDEF MSWindows}
  if Columns.Count > 1 then begin
    for i := 0 to Items.Count - 1 do
    begin
      for j := 0 to Items[i].SubItems.Count - 1 do
      begin FillChar(LV, SizeOf(LV), 0);
        LV.mask := LVIF_TEXT;
        LV.iItem := i;
        LV.iSubItem := j + 1;
        txt := Items[i].SubItems[j];
        LV.pszText := PWideChar(txt);
        SendMessage(Handle, LVM_SETITEMW, 0, LPARAM(@LV));
      end;
    end;
  end;
