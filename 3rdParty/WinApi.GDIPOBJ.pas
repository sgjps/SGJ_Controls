...
begin
  unit_ := UnitWorld;
  nativePen := nil;
  lastResult := GdipCreatePen2(brush.FNativeBrush, width, unit_, nativePen);
end;
...
begin
  result := SetStatus(GdipSetPenBrushFill(nativePen, brush.FNativeBrush));
end;
...
begin
  type_ := TBrushType(-1);
  SetStatus(GdipGetBrushType(FNativeBrush, type_));
  result := type_;
end;
...
begin
  SetStatus(GdipGetLineGammaCorrection(GpLineGradient(FNativeBrush),
    useGammaCorrection));
...
begin
  ...
  if assigned(brush) then nbrush := brush.FNativeBrush else nbrush := nil;
...
begin
  ...
  SetStatus(GdipGetPathGradientSurroundColorCount(GpPathGradient(FNativeBrush), count1));
  ...
  SetStatus(GdipGetPathGradientSurroundColorsWithCount(GpPathGradient(FNativeBrush), colors, count1));
  if (lastResult = Ok) then
    count := count1;
...
begin
  ...
  SetStatus(GdipSetPathGradientSurroundColorsWithCount(
    GpPathGradient(FNativeBrush), colors, count1));
...
begin
  SetStatus(GdipGetPathGradientGammaCorrection(GpPathGradient(FNativeBrush), result));
end;
...
begin
  result := SetStatus(GdipGetPathGradientTransform(GpPathGradient(FNativeBrush),
    matrix.nativeMatrix));
end;
